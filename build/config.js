...
const aliases = {
  ...
  he: path.resolve(
    __dirname,
    '..',
    'platform/nativescript/util/entity-decoder'
  ),
}
...
const builds = {
  'nativescript-vue': {
    ...
    external(id) {
      return (
        id.startsWith('@nativescript') ||
        id.startsWith('tns-core-modules') ||
        id.startsWith('weex')
      )
    },
  },
  'nativescript-vue-template-compiler': {
    ...
    dest: './packages/nativescript-vue-template-compiler/index',
    moduleName: 'NativeScript-Vue-Template-Compiler',
    banner: banner('NativeScript-Vue-Template-Compiler'),
    external(id) {
      const deps = Object.keys(
        require('../packages/nativescript-vue-template-compiler/package.json')
          .dependencies
      )
      return (
        deps.includes(id) ||
...
const plugins = [
  replace({
    ...
    'devtools && config.devtools':
      'global.__VUE_DEVTOOLS_GLOBAL_HOOK__ && config.devtools',
    "devtools.emit('flush')":
  ...
  const config = [
    {
      ...
      onwarn(warning) {
        if (
          ...
          return
        } else if (warning.message.includes('weex')) {
          return
        }
