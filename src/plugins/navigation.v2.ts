import { App, Component, h, isRef, Ref } from "@vue/runtime-core";
import { Frame, NavigationEntry, Page } from "@nativescript/core";
import { NSVElement, NSVRoot, renderInRootAppContext } from "..";
...
function createNavigationRoot(cb: (view: any) => void) {
  ...
  defaultRoot.removeChild = (el) => {
    ...
    console.log("NAV ROOT remove child", (el as NSVElement).tagName);
  };
  ...
  defaultRoot.appendChild = (el) => {
    ...
    if (shouldResetRoot) {
      ...
      console.log("NAVIGATION ROOT APPEND CHILD");
  ...
  try {
    ...
    const cleanup = (page) => {
      if (page === latestPage) {
        ...
        // unmounts the target component...
        renderInRootAppContext(null, navigationRoot);
      } else {
        console.log("no dispose we have replaced page");
      }
    };

    const navigationRoot = createNavigationRoot((newPage) => {
      latestPage = newPage;
    ...
    // mounts the target component to the navigationRoot
    renderInRootAppContext(h(target as any, options.props), navigationRoot);

    const targetPage = navigationRoot.el.nativeView as Page;
